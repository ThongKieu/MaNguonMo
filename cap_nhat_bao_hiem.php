...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf(
    "UPDATE tlb_baohiem SET so_bhxh=%s, ngay_cap_bhxh=%s, noi_cap_bhxh=%s, so_bhyt=%s, ngay_cap_bhyt=%s, noi_cap_bhyt=%s WHERE ma_nhan_vien=%s",
  ...
  <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
    <table class="row2" width="800" align="center" cellpadding="3" cellspacing="3" bgcolor="#66CCFF">
      ...
      <tr valign="baseline">
        ...
        <td><input type="text" name="ngay_cap_bhxh" value="<?php $sqldate = strtotime($row_RCBaohiem_CN['ngay_cap_bhxh']);
                                                            $date = date('d-m-Y', $sqldate);
                                                            echo $date; ?>" size="32" /></td>
        <td>Ngày cấp BHYT:</td>
        <td><input type="text" name="ngay_cap_bhyt" value="<?php $sqldate2 = strtotime($row_RCBaohiem_CN['ngay_cap_bhyt']);
                                                            $date2 = date('d-m-Y', $sqldate2);
                                                            echo $date2; ?>" size="32" /></td>
      </tr>
      <tr valign="baseline">
